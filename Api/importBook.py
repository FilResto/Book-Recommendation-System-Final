...
def clean_price(price_str):
    """Clean and convert the string to a valid price."""
    try:
...
def import_books():
    ...
    with app.app_context():
        for _, row in df.iterrows():
            ...
            if BookModel.query.get(row['bookId']):
                print(f"The book with ID {row['bookId']} already exists. Skipped.")
                continue

            # Create the BookModel object
            book = BookModel(
                id=row['bookId'],
                title=row['title'],
    ...
    print(f"{len(books)} books imported successfully!")
