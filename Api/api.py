...
class UserModel(db.Model, UserMixin):
    ...
    @property
    def is_active(self):
        # Returns True if the user is active. Change this logic if necessary.
        return True
...
def get_books():
    ...
    # Variable to store the sorted order of books
    sorted_books = []

    # Logic based on the sorting criterion
    if sort_criteria == "Content Based Filtering":
        recommendations = cbf(user_id)  # Call the Content-Based Filtering function
    elif sort_criteria == "Collaborative Filtering Userbased":
        recommendations = cfi(user_id)  # Call User-based CF
    elif sort_criteria == "Collaborative Filtering Itembased":
        recommendations = cfu_single_user(user_id)  # Call Item-based CF
    elif sort_criteria == "Q-Learning":
        recommendations = qlearning(user_id)  # Call Q-Learning
    # elif sort_criteria == "DQN":
    #     recommendations = dqn_recommendations(user_id)  # Call DQN
    else:
        return jsonify({"error": f"Invalid sortCriteria: {sort_criteria}"}), 400

    # Sort books based on recommendations
    if recommendations:
        book_id_order = recommendations  # The IDs of the sorted books
        # Sort books by their order in the recommendations
        sorted_books = sorted(books_data, key=lambda x: book_id_order.index(x['id']) if x['id'] in book_id_order else len(book_id_order))
    else:
        # If no recommendation, return books without specific order
...
def get_book_detail():
    ...
    # Search for ratings related to the book
    ...
    # Example of a logged-in user (replace with real logic)
    current_user_id = 1
    user_rating = next(
...
# Export the model to create the tables
